...
ggplot(subset(loanData, InquiriesLast6Months > 0 & !is.na(CreditScoreRanges)), 
       aes(x = InquiriesLast6Months, color = CreditScoreRanges)) +
  geom_bar(aes(fill=CreditScoreRanges)) +
  scale_x_continuous(limits = c(0,16), breaks = seq(1,15,1)) +
  ggtitle("Credit Inquiries in the Last 6 Months") +
...
However, borrowers with credit ratings over 700 still show up in the graph 
around 5 inquiries.
...
ggplot(subset(loanData, CurrentDelinquencies > 0 & !is.na(CreditScoreRanges)), 
       aes(x = CurrentDelinquencies, color = CreditScoreRanges)) +
  geom_bar(aes(fill=CreditScoreRanges)) +
  scale_x_continuous(limits = c(0,16), breaks = seq(1,15,1)) +
...
majority of delinquencies are coming from borrowers with sub-700 credit scores.
...
ggplot(subset(loanData, DebtToIncomeRatio < 1.5 & CreditScoreRangeLower > 400), 
       aes(x = CreditScoreRangeLower, y = DebtToIncomeRatio)) +
...
# Plot a histogram of Defaulted Loans by Origination date and bucket by
# Credit Score Range
ggplot(subset(loanData, !is.na(CreditScoreRanges) & 
  ...
  geom_histogram(bins = 60, aes(fill=CreditScoreRanges), color='black') +
...
ggplot(subset(loanData, CreditScoreRangeLower > 400 & BankcardUtilization < 1.1), 
       aes(x = CreditScoreRangeLower, y = BorrowerRate, color = BankcardUtilization)) +
  geom_jitter() 
...
ggplot(subset(loanData, CreditScoreRangeLower > 400 & 
                BankcardUtilization < 1.1 & 
                str_detect(LoanStatus, 'Past Due')==TRUE), 
       aes(x = CreditScoreRangeLower, y = BorrowerRate, 
           color = BankcardUtilization)) +
  geom_jitter() +
...
# Plot a dotplot of loan origination date and borrower rate, with color for
# Credit scores and separate graphs for each term
ggplot(subset(loanData, !is.na(CreditScoreRanges)), 
...
insight on what we can expect based on historical examples.
...
The correlation between credit scores and borrower rates shows that there is a 
negative correlation between the two.  This makes sense as we would expect that 
